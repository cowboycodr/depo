...
<div class="grid h-full min-h-0 grid-rows-[42px_minmax(0,1fr)] bg-bg text-fg">
  <header class="flex h-10.5 shrink-0 items-center border-b border-line px-4">
    <a href="/" class="flex items-center gap-2 text-ui-md font-medium text-fg-bright">
      ...
      >
        <defs>
          <linearGradient id="depo-home" x1="13" y1="8" x2="9" y2="15" gradientUnits="userSpaceOnUse">
        ...
        <polygon points="9,1 17,15 1,15" class="fill-none stroke-logo-stroke stroke-[1.1]" stroke-linejoin="round" />
        <path d="M13 8 L17 15 L9 15" class="fill-none stroke-[1.1]" stroke="url(#depo-home)" stroke-linecap="round" stroke-linejoin="round" />
      </svg>
      <span>Depo</span>
    </a>
  </header>
  ...
  <main class="min-h-0 overflow-y-auto bg-canvas px-5 py-5">
    <div class="mx-auto grid max-w-5xl gap-4 lg:grid-cols-[260px_minmax(0,1fr)]">
      ...
      <section class="rounded-ui bg-surface-muted p-4 shadow-ring">
        ...
        {#if form?.error}
          <div class="mb-3 flex items-start gap-1.5 rounded-ui bg-bg px-2.5 py-2 text-ui text-danger shadow-ring">
      ...
      <section class="rounded-ui bg-surface-muted p-4 shadow-ring">
            ...
            {#each data.repos as repo (repo.id)}
              ...
              >
                <span class="min-w-0">
                  ...
                  <span class="mt-0.5 inline-flex items-center gap-1.5 font-mono text-ui-xs text-fg-ref">
        ...
        {:else}
          <div class="flex h-32 items-center justify-center rounded-ui bg-bg text-ui text-fg-secondary shadow-ring">
            No repositories yet.
          </div>
        {/if}
      </section>
