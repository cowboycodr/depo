<script lang="ts">
  ...
  const repoHref = (owner: string, name: string): AppHref =>
    appHref(`/${encodeURIComponent(owner)}/${encodeURIComponent(name)}`);
</script>
...
<div class="grid h-full min-h-0 grid-rows-[42px_minmax(0,1fr)] bg-bg text-fg">
  <header class="flex h-10.5 shrink-0 items-center border-b border-line px-4">
    <a href={resolve('/')} class="flex items-center gap-2 text-ui-md font-medium text-fg-bright">
      <svg
          ...
          <div class="grid gap-1.5">
            {#each data.repos as repo (repo.id)}
              <a
                ...
                href={resolve(repoHref(repo.owner, repo.name))}
              >
                <span class="min-w-0">
