<script lang="ts">
  ...
  const lineCount = $derived(content.length > 0 ? content.split('\n').length : 0);
</script>
...
<div class="grid h-full grid-rows-[42px_minmax(0,1fr)]">
  ...
  <Sidebar.Root bind:open={sidebarOpen}>
    <div class="flex h-[calc(100vh-42px)] overflow-hidden bg-canvas">
      <Sidebar.Panel>
        <RepositoryTree nodes={data.view?.tree.nodes ?? []} selectedPath={activePath} />
      </Sidebar.Panel>
      ...
      <main class="relative min-w-0 flex-1 overflow-hidden">
        ...
        >
          <FileHeader
            bind:diffStyle
            fileName={activeFile?.path ?? activePath}
            lines={lineCount}
            size={activeFile?.size ?? 0}
            additions={activeFile?.kind === 'text' ? lineCount : 0}
          />
          <div class="flex min-h-0 flex-1 flex-col overflow-hidden">
            ...
            {:else if activeFile?.kind === 'text' && activeFile.content !== null}
              {#key `${diffStyle}:${activeFile.commitSha}:${activeFile.path}`}
                <DiffViewer
                  {diffStyle}
